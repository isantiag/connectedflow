...
class SocketCANAdapter extends EventEmitter {
  ...
  async checkInterface() {
    return new Promise((resolve) => {
      exec(`ip link show ${this.interfaceName} 2>/dev/null`, (err, stdout) => {
        if (err || !stdout) return resolve({ available: false, reason: `Interface ${this.interfaceName} not found` });
  ...
  async send(canId, data) {
    const hexId = typeof canId === 'number' ? canId.toString(16).toUpperCase().padStart(8, '0') : canId;
    const hexData = Buffer.isBuffer(data) ? data.toString('hex').toUpperCase() : data;
    return new Promise((resolve, reject) => {
      exec(`cansend ${this.interfaceName} ${hexId}#${hexData}`, (err) => {
        if (err) reject(new Error(`cansend failed: ${err.message}`));
